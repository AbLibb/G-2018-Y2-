...
<div>
	<label class="shou_la">草稿箱</label>(共 <label id="count_cao" class="text-danger"></label> 封)
</div>
...
<script>
	...
	$("[name=wheres]").blur(function(){
		Page_Text(1,2,this.value);
	});
	$(".ddd_4").on("change",function(){
	...
	function LoadFile(){
		$.get("/Email/Home/showEmailFile","",function(data){
			$(".ddd_4").html("");
			$(".ddd_4").append("<option>---请选择---</option>");
			$.each(data,function(){
				if(this.euid!=null){
			...
			$(".ddd_4").append("<option>-----------------</option>");
			$(".ddd_4").append("<option class='xinjian'>自定义文件夹</option>");
		});
	}
	//删除
	function todelect() {
	...
	function TextCount(){
		$.get("/Email/Home/yiduss",{"efid":6},function(data){
			$("#count_cao").text(data);
		});
	}
	function Page_Text(begin,size,title){
		//提交
		$(".list-All_Text").html("");
				...
				if(json.data==""){
					...
					$.each(json.data,function(i,obj){
						...
						$.each(obj.eemailList,function(i,dd){
							...
							if(obj.scicemail==1){
								$tr = "<tr><td><input type='checkbox' value="+obj.erid+" name='two'></td><td><lable class='text-danger'>密送</lable></td><td>"+obj.euser.uname+"</td><td  colspan='3'>"+dd.content+"</td>\<td>"+obj.readdate+"</td>\</tr>";
								$(".list-All_Text").append($tr);
							}else{
								$tr = "<tr><td><input type='checkbox' value="+obj.erid+" name='two'></td><td><lable class='text-danger'>发送</lable></td><td>"+obj.euser.uname+"</td><td  colspan='3'>"+dd.content+"</td>\<td>"+obj.readdate+"</td>\</tr>";
								$(".list-All_Text").append($tr);
							}
						})
					})
				 ...
				 $("[name=next]").attr("onclick","Page_Text("+json.prev+",2,'"+title+"')");
                 $("[name=prev]").attr("onclick","Page_Text("+json.next+",2,'"+title+"')");
		}
		});
	}
</script>