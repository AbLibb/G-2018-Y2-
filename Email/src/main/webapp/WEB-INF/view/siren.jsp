...
<div>
	<label class="shou_la nx"></label>(共 <label class="siren"></label> 封)
</div>
			...
			<div class="modal-dialog modal-sm" style="width: 50rem;">
				<div class="modal-content">
					...
					<div class="modal-footer">
						...
						<button class="btn btn-primary"   onclick="submits_siren()">确认</button>
					</div>
				</div>
			</div>
		</div>
...
<script>
	...
	$(function(){
		...
		getName(ss);
		LoadFile();
		$.get("/Email/Home/yiduss",{"efid":ss},function(data1){
			$(".siren").text(data1);
	...
	$("[name=wheres]").blur(function(){
		Page_siren(1,2,this.value);
	});
	$(".ddd_5").on("change",function(){
	...
	function getName(efid){
		$.post("/Email/Home/getFoldername",{"efid":efid},function(data){
			$(".nx").text(data.Foldername);
		});
	}
	...
	function LoadFile(){
		...
		$.get("/Email/Home/showEmailFile","",function(data){
			$(".ddd_5").append("<option>---请选择---</option>");
			$.each(data,function(){
			...
			$(".ddd_5").append("<option>-----------------</option>");
			$(".ddd_5").append("<option class='xinjian'>自定义文件夹</option>");
		});
	}
	//删除
	function todelect() {
	...
	function Page_siren(begin,size,title,efid){
		//提交
		$(".list-All_siren").html("");
		$.ajax("/Email/Home/GetPage", {
			...
			success: function(json) {
				...
				if(json.data==""){
					...
					$.each(json.data,function(i,obj){
						$.each(obj.eemailList,function(i,dd){
							...
							if(obj.emailstatus==0){	
								if(obj.scicemail==1){
									$tr = "<tr><td><input type='checkbox' value="+obj.erid+" name='two'></td><td><lable class='text-danger'>未读</lable></td><td>*****</td><td  colspan='3'>"+dd.content+"</td>\<td>"+obj.readdate+"</td>\</tr>";
									$(".list-All_siren").append($tr);
								}else{
									$tr = "<tr><td><input type='checkbox' value="+obj.erid+" name='two'></td><td><lable class='text-danger'>未读</lable></td><td>"+obj.euser.uname+"</td><td  colspan='3'>"+dd.content+"</td>\<td>"+obj.readdate+"</td>\</tr>";
									$(".list-All_siren").append($tr);
								}
							}else{
								if(obj.scicemail!=1){
									$tr = "<tr><td><input type='checkbox' value="+obj.erid+" name='two'></td><td><lable class='text-danger'>已读</lable></td><td>"+obj.euser.uname+"</td><td  colspan='3'>"+dd.content+"</td>\<td>"+obj.readdate+"</td>\</tr>";
									$(".list-All_siren").append($tr);
								}else{
									$tr = "<tr><td><input type='checkbox' value="+obj.erid+" name='two'></td><td><lable class='text-danger'>已读</lable></td><td>******</td><td  colspan='3'>"+dd.content+"</td>\<td>"+obj.readdate+"</td>\</tr>";
									$(".list-All_siren").append($tr);
								}
							}
						})
				 ...
				 $("[name=next]").attr("onclick","Page_siren("+json.prev+",2,'"+title+"',"+efid+")");
                 $("[name=prev]").attr("onclick","Page_siren("+json.next+",2,'"+title+"',"+efid+")");
		}
		});
	}
</script>